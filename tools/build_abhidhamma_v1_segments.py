...
RAW_DIR = PROJECT_ROOT / "data" / "อภิธรรมฯ" / "บาลี" / "(ไฟล์ดิบ) อภิธมฺมตฺถสงฺคหปาลิยา (สมบูรณ์)"
OUTPUT_DIR = PROJECT_ROOT / "data" / "อภิธรรมฯ" / "segments"
OUTPUT_PATH = OUTPUT_DIR / "อภิธมฺมตฺถสงฺคหปาลิ.segments.jsonl"

BOOK = "อภิธรรมฯ"
SOURCE = "อภิธมฺมตฺถสงฺคหปาลิ"
VOLUME_NO = 1
MAX_PAGE = 279
RAW_CHUNK_BASE_NAME = "อภิธมฺมตฺถสงฺคหปาลิ"
...
def chunk_filename_for_page(page: int) -> str:
    ...
    return f"{RAW_CHUNK_BASE_NAME} - หน้า {range_part}{suffix}.txt"
...
def classify_segment_type(text: str) -> str:
    ...
    if re.fullmatch(r"-{3,}", s):
        return "separator"
    if re.search(r"วณฺณนา$", s) or re.search(r"ปริจฺเฉโท$", s):
        return "heading"
    return "content"
...
def iter_segments() -> list[Segment]:
    ...
    def add_segment(text: str, page_start: int, page_end: int) -> None:
        ...
        if not cleaned:
            return
        seg_index += 1
        seg_id = f"abhidhamma-v{VOLUME_NO}-s{seg_index:06d}"
