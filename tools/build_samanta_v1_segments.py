...
RAW_DIR = PROJECT_ROOT / "data" / "สมันตปาสาทิกา" / "บาลี" / "(ไฟล์ดิบ) สมนฺตปาสาทิกา นาม วินยฏฺฐกถา (๑) ปฐโม ภาโค (สมบูรณ์)"
OUTPUT_DIR = PROJECT_ROOT / "data" / "สมันตปาสาทิกา" / "segments"
OUTPUT_PATH = OUTPUT_DIR / "สมนฺตปาสาทิกา_01.segments.jsonl"

BOOK = "สมันตปาสาทิกา"
SOURCE = "สมนฺตปาสาทิกา"
VOLUME_NO = 1
MAX_PAGE = 630
RAW_CHUNK_BASE_NAME = "สมนฺตปาสาทิกา"
...
def chunk_filename_for_page(page: int) -> str:
    ...
    return f"{RAW_CHUNK_BASE_NAME} ({vol_thai}) - หน้า {range_part}{suffix}.txt"
...
def iter_segments() -> list[Segment]:
    ...
    def add_segment(text: str, page_start: int, page_end: int) -> None:
        ...
        if not cleaned:
            return
        seg_index += 1
        seg_id = f"samanta-v{VOLUME_NO}-s{seg_index:06d}"
